...
export const getServices = () => dispatch => {
    ...
    axios.get(`https://cors-anywhere.herokuapp.com/${SERVICE_URL}/?key=${SERVICE_KEY}&action=services`)
        .then(res => dispatch({ type: GET_INSTAGRAM_SERVICES_SUCCESS, payload: res.data }))
        .catch(err => dispatch({ type: GET_INSTAGRAM_SERVICES_FAILURE, payload: err }))
}

// ----------------------------- SEND INSTAGRAM SERVICES ----------------------------- 

export const SEND_INSTAGRAM_SERVICES_START = "SEND_INSTAGRAM_SERVICES_START";
export const SEND_INSTAGRAM_SERVICES_SUCCESS = "SEND_INSTAGRAM_SERVICES_SUCCESS";
export const SEND_INSTAGRAM_SERVICES_FAILURE = "SEND_INSTAGRAM_SERVICES_FAILURE";

export const sendServices = options => dispatch => {
    dispatch({ type: SEND_INSTAGRAM_SERVICES_START })
    axios.get(`https://cors-anywhere.herokuapp.com/${SERVICE_URL}/?key=${SERVICE_KEY}&action=add&service=475&quantity=${Number(options.quantity)}&link=${options.link}`)
        .then(res => dispatch({ type: SEND_INSTAGRAM_SERVICES_SUCCESS, payload: res.data }))
        .catch(err => dispatch({ type: SEND_INSTAGRAM_SERVICES_FAILURE, payload: err }))
}